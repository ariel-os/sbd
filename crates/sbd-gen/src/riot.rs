...
fn generate_riot_target(sbd: &SbdFile, target: &Target) -> Result<RiotTarget> {
    ...
    // SPIs
    {
        ...
        if !spi_buses.is_empty() {
            ...
            for (bus_n, spi_bus) in spi_buses.iter().enumerate() {
                #[expect(clippy::unnecessary_find_map)]
                ...
                if let Some(key) = peripheral_key {
                    ...
                    let bus_upper = format!("SPI{bus_n}");
                    for (dev_n, device) in spi_bus.devices.iter().enumerate() {
                        let device_upper = format!("DEVICE{dev_n}");
                        let cs_pin = name2riot_pin(&device.cs)?;
                        all_cs_pins.push((bus_upper.clone(), device_upper.clone(), cs_pin));
                        if let Some(drdy) = &device.drdy {
                            let drdy_pin = name2riot_pin(drdy)?;
                            all_drdy_pins.push((bus_upper.clone(), device_upper, drdy_pin));
                        }
                    }
            ...
            if !spis_configured.is_empty() {
                ...
                for (bus_upper, device_upper, drdy_pin) in &all_drdy_pins {
                    let _ = writeln!(
                        s,
                        "#define SPI_DRDY_{bus_upper}_{device_upper}  ({drdy_pin})"
                    );
                }
